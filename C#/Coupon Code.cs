...
public static class Kata
{
  ...
  {
    var newCurrent = Convert.ToDateTime(currentDate);
    var newExpiry = Convert.ToDateTime(expirationDate);
    int result = DateTime.Compare(newCurrent, newExpiry);
    if (enteredCode == correctCode && result <= 0) {
      return true;
    }
    else {
      return false;
